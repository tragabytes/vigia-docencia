...
/* Typing del título con un typo intencional y corrección, tipo terminal.
   Secuencia: teclea "vigia-docente", pausa breve, borra "nte" y corrige a
   "ncia" para llegar a "vigia-docencia" (guiño docente → docencia). */
function typeTitle() {
  const target = 'vigia-docencia';
  const wrong  = 'vigia-docente';         // typo en la última sílaba: docente → docencia
  const keep   = 'vigia-doce';            // hasta donde retrocede el cursor
...
function subSubmit() {
  ...
  subState.step = 3;
  drawSub();
  // Note: real impl would POST to https://api.vigia-docencia.workers.dev/api/subscribe
}
...
<span class="dim">└────────────────────────────────────────────────────────────┘</span>`;
  ...
  const repoUrl = 'https://github.com/tragabytes/vigia-docencia';
...
function renderFooter() {
  $('#footer').innerHTML = `
    ...
    <span class="sep">│</span>
    <span><a href="https://github.com/tragabytes/vigia-docencia" target="_blank">github.com/tragabytes/vigia-docencia →</a></span>
    <span class="sep">│</span>
